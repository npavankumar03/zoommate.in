  ...
  switch (transition) {
    ...
    default:
      ...
      return [
        ...
        "- If the follow-up is too ambiguous, ask ONE clarifying question.",
      ].join("\n");
  }
}
...
export async function* streamAssistantAnswer(
  ...
  try {
    ...
    const candidateSpan = extractCandidateSpan(effectiveQuestion) || effectiveQuestion.trim();
    ...
    const ackOnly = /^(yes|yeah|yep|yup|correct|right|sure|i do|i did|absolutely|of course|exactly)$/.test(ackReplyNorm);
    ...
    const relevantPairs = followUp.isFollowUp
      ...
      : vagueQuestion
        ? selectRelevantQAPairs(meetingId, effectiveQuestion, 8, 3200) // vague → full history scan
    ...
    const tier0MemoryContext = cachedMem
      ? cachedMem
      : await formatMemorySlotsForPrompt(userId, meetingId).then((v) => {
          setCached(memoryContextCache, memCacheKey, v || "", MEMORY_CACHE_TTL_MS);
          return v || "";
        });

    // Inject past session insights (gaps, weak areas, improvement notes from previous sessions)
    const sessionInsights = await formatSessionReviewsForPrompt(userId).catch(() => "");
    ...
    if (followUp.isFollowUp) {
      ...
      const anchorA = (anchor.lastAssistantAnswer || relevantPairs[0]?.a || "").slice(0, 2000);
    ...
    if (!fullAnswer.trim()) {
      fullAnswer = "I need a bit more context to answer that precisely. Could you clarify the question?";
      totalChunks += 1;
