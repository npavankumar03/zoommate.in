...
export interface OverlayState {
  transcript:           string;  // latest interim STT text
  finalTranscript:      string;  // last committed sentence
  question:             string;  // question detected by orchestrator
  answer:               string;  // accumulated streamed answer
  isStreaming:          boolean;
...
export function useZoommateSocket(meetingId: string) {
  ...
  useEffect(() => {
    ...
    // ── Answer streaming ───────────────────────────────────────────────────
    socket.on("response_start", ({ displayQuestion }: any) => {
      setState((s) => ({
        ...s,
    ...
    socket.on("answer", ({ delta }: { delta: string; requestId: string }) => {
      setState((s) => ({
        ...s,
    ...
    socket.on("response_end", ({ error, cancelled }: any) => {
      setState((s) => ({
        ...s,
        isStreaming:          false,
        isAwaitingFirstChunk: false,
    ...
    socket.on("answer_cached", ({ answer, question }: any) => {
      setState((s) => ({
        ...s,
    ...
    (async () => {
      try {
        ...
        recognizer.startContinuousRecognitionAsync(
          ...
          (err) => setState((s) => ({ ...s, sttError: `Mic error: ${err}` }))
        );
      } catch (err: any) {
        if (!destroyed) {
